	  ...
	  FROM (SELECT PLAYER_NAME,
		   ...
		   (SELECT shotdata1.PLAYER_NAME,
			...
			FROM (SELECT PLAYER_NAME
			     ...
			     (SELECT shotData.PLAYER_NAME, CASE WHEN UPPER(shotData.ACTION_TYPE) LIKE '%BANK Shot%'
                                                          THEN 'BANK SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%DUNK SHOT%'
                                                          THEN 'DUNK SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%FADEAWAY SHOT%'
                                                          THEN 'FADEAWAY SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%HOOK SHOT%'
                                                          THEN 'HOOK SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%JUMP SHOT%'
                                                          THEN 'JUMP SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%LAYUP SHOT%'
                                                          THEN 'LAYUP SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%NO SHOT%'
                                                          THEN 'NO SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%TIP SHOT%'
                                                          THEN 'TIP SHOT'
                                                          WHEN UPPER(shotData.ACTION_TYPE) LIKE '%ROLL SHOT%'
                                                          THEN 'ROLL SHOT'
                                                          ELSE UPPER(shotData.ACTION_TYPE)
                                                      END AS ACTION_TYPE
